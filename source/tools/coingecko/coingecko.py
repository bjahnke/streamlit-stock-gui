...
from source.code.coingecko import get_products, get_all_products, build_category_product_relation
...
if st.button('Build Category Product Relation', help='Build a relation between categories and products for all products listed on coingecko. Store in your connected database'):
    with st.spinner('Building category product relation...'):
        product_category_rel = build_category_product_relation()
        product_category_rel.to_sql('coin_gecko_category_product_relation', con=st.session_state.db_con, if_exists='replace', index=False)
